...
本格的な競合リサーチ（異常値分析・バズ方程式モデリング）が必要なときは、前段スキル `clbs-youtube-research` の `04_research_summary.yaml` を渡せます。
...
clbs-youtube-script-pro（企画考案＋台本）→ HeyGenで video.mp4 生成 → clbs-youtube-edit（編集）→ 完成動画
```
